    ...
    methods: {
      ...
      loadData() {
        ...
        if (id != null) {
          ...
          this.httpPost("/video/courseType/queryByCourseTypeId", param).then((res) => {
            this.form = res.data;
            this.indexType = type;
            if (type == 3) {
      ...
      onCourseTypeSubmit: function () {
        this.$refs["form"].validate((valid) => {
          if (valid) {
            this.httpPost("/video/courseType/add", this.form).then((res) => {
              this.$message({
                message: '新增课程类型成功！',
                type: 'success'
              });
              this.$router.push("/index/courseTypeManage");
            });
          }
        })
      },
      ...
      onCourseTypeUpdate(){
        this.$refs["form"].validate((valid) => {
          if (valid) {
            ...
            this.httpPost("/video/courseType/update", this.form).then((res) => {
              this.$message({
                message: '修改视频成功！',
                type: 'success'
              });
