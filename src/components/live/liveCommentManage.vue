<template>
  <div class="app-container">
    <div class="block">
      ...
      <!--评论表格-->
      <el-table :data="liveCommentList" border style="width: 100%">
        <el-table-column type="index" label="序号" width="50">
        </el-table-column>
        <el-table-column prop="liveTitle" label="直播名称">
        </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      loadData() {
        ...
        this.httpPost("/live/liveComment/list", param).then((res) => {
          this.liveCommentList = res.data.data;
      ...
      handleExamineComment(id) {
        this.$confirm('此操作将审核评论, 是否继续?', '提示', {
          ...
          this.httpPost("/live/liveComment/examine", param).then((res) => {
            this.$message({
              message: '审核评论成功！',
              type: 'success'
            });
      ...
      handleExamineNotPassComment(id) {
        this.$confirm('此操作将不通过审核评论, 是否继续?', '提示', {
          ...
          this.httpPost("/live/liveComment/examineNotPass", param).then((res) => {
            this.$message({
              message: '审核不通过评论成功！',
              type: 'success'
            });
